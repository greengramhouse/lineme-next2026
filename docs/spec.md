...
## ⏸ พักไว้ — ระบบฝากไฟล์ (2026-08-16)

ไอเดีย: ครูพิมพ์ "ฝากไฟล์" → บอทให้ส่งไฟล์ → เก็บขึ้น cloud แยกโฟลเดอร์ตามคนส่ง
พิมพ์ "ขอไฟล์" → บอทแสดงรายการไฟล์ที่เคยฝากไว้

**ยังไม่ได้เขียนโค้ดเลย** แต่หาข้อมูลไว้แล้ว กลับมาทำต่อได้โดยไม่ต้องเริ่มใหม่

### สิ่งที่ตรวจสอบแล้ว

- ✅ LINE ส่ง `fileName` และ `fileSize` มากับ event ชนิด `file` — เก็บชื่อเดิมได้
- ✅ ดาวน์โหลดไฟล์ใช้ `lineBlobClient.getMessageContent()` ซึ่ง **มีโค้ดอยู่แล้ว** ใน audio path
- ✅ state "รอรับไฟล์" ใช้ `ConversationState` ตัวเดียวกับระบบร่างเอกสารได้เลย

### 🔴 ข้อจำกัดที่ตัดสินใจไปแล้ว ห้ามลืม

1. **Service Account ของ Google มีโควตา 0 GB** ตั้งแต่ 1 มิ.ย. 2023 และขอเพิ่มไม่ได้
   → อัปโหลดเข้า Drive ของ service account **ใช้ไม่ได้** จะได้ `403 storageQuotaExceeded`
2. **บัญชี `@saraburi2.go.th` สร้าง Shared Drive ไม่ได้** — หน้า `/drive/shared-drives` เปิดได้
   (แปลว่าโดเมนเปิดฟีเจอร์) แต่ไม่มีปุ่มสร้าง = ผู้ดูแลโดเมนปิดสิทธิ์สร้างไว้
   → ถ้าจะใช้ Shared Drive ต้องขอ IT ของ สพป. สร้างให้แล้วใส่เราเป็นผู้จัดการ
3. **ถ้าไปทาง OAuth ต้องเลือก "ภายใน (Internal)"** ถ้าเลือก External แล้วปล่อยสถานะ
   "กำลังทดสอบ" **refresh token จะตายใน 7 วัน** ต้องกด "เผยแพร่แอป" ด้วย
4. **ใช้ scope `drive.file` เท่านั้น** (ไม่ใช่ `drive`) เพราะ `drive` เป็น restricted scope
   ต้องผ่านการตรวจสอบของ Google หลายสัปดาห์
   ผลข้างเคียง: แอปเห็นเฉพาะไฟล์ที่ตัวเองสร้าง → **สร้างโฟลเดอร์แม่เองใน Drive ไม่ได้**
   ต้องให้แอปสร้างเองตอนรันครั้งแรก
5. **LINE ส่งไฟล์แนบทั่วไปกลับหาผู้ใช้ไม่ได้** (pdf/docx/xlsx)
   ตอน "ขอไฟล์" ต้องตอบเป็นรายการพร้อมลิงก์ ยกเว้นรูปภาพที่ส่งกลับเข้าแชทได้
6. ไฟล์ถูกโหลดเข้าหน่วยความจำทั้งก้อนก่อนอัปโหลด → ควรจำกัดขนาด (~20 MB)
   ไม่งั้นชนลิมิตหน่วยความจำและ `maxDuration = 60` ของ Vercel

### ทางเลือกที่ยังไม่ได้ตัดสินใจ

| | Google Drive | Supabase Storage |
|---|---|---|
| ตั้งค่า | ~20 นาที + กับดัก 5 ข้อข้างบน | ~2 นาที (มี Supabase อยู่แล้ว) |
| ครูจัดการไฟล์เองได้ | ✅ เปิดแอป Drive ได้เลย | ❌ ต้องผ่านบอท/dashboard |

> **ถ้ากลับมาทำ:** แยกชั้นเก็บไฟล์เป็น interface เดียว (`storageService`) ตั้งแต่แรก
> จะได้สลับ Supabase ↔ Drive ได้โดยไม่ต้องแตะ flow และหน้า dashboard
...
**ค้างอยู่:**
1. ⬜ **revoke Typhoon key เดิม** (ข้อ 5.2b) — *งานเก็บกวาด ทำเมื่อสะดวก ไม่เร่ง*
   key เดิมเป็น playground key ที่ไม่ได้ผูกบัตร ผลกระทบสูงสุดคือโควตาของบัญชีถูกใช้ไปเฉย ๆ
   และตอนนี้โค้ดไม่ได้ใช้ key ตัวนั้นแล้ว (อ่านจาก env และเปลี่ยนเป็นตัวใหม่ไปแล้ว)
...
- [x] **4.11** **M11** — ลบ dead code `if (!TYPHOON_API_KEY)` (ตอนนั้นเงื่อนไขเป็นจริงไม่ได้เพราะ key เป็น literal)
      ต่อมาในข้อ 5.1 ย้ายไปอ่านจาก env แล้ว เงื่อนไขนี้จึงกลับมามีความหมายและถูกเขียนใหม่
...
>
> **ระดับความรุนแรงจริง (เจ้าของยืนยัน 2026-08-16):** key ตัวนั้นเป็น **playground key ที่ไม่ได้ผูกบัตร**
> ผลกระทบสูงสุดคือโควตาของบัญชีถูกใช้ไปเฉย ๆ ไม่ใช่เรื่องเงินหรือข้อมูลรั่ว
> จึงเป็นงานเก็บกวาด ไม่ใช่เรื่องเร่งด่วนอย่างที่ประเมินไว้ตอนแรก
...
- [ ] **5.2b** revoke key เดิม — *งานเก็บกวาด ทำเมื่อสะดวก*
      key เดิมยังดึงได้จาก GitHub ที่ commit `074df3c` (ตรวจแล้วเมื่อ 2026-08-16 ยังพบอยู่ 1 บรรทัด)
      และยังใช้งานได้จนกว่าจะ revoke แต่โค้ดไม่ได้ใช้ key ตัวนั้นแล้ว จึงไม่มีอะไรพังถ้ายังไม่ทำ
      > ⚠️ **จุดเดียวที่ต้องจำ:** ถ้าวันหนึ่งอัปเกรดบัญชี Typhoon เป็นแบบเสียเงิน
      > key เก่าที่ยังไม่ถูก revoke จะกลายเป็นเรื่องขึ้นมาทันที เพราะมันอยู่ใน git history ตลอดไป
      > — ให้ revoke **ก่อน** อัปเกรดบัญชี
